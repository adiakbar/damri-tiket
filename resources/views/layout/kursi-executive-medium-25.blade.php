<div class="container-kursi" data-kode-trayek="{{ $bis->kode_trayek }}" data-nomor-bis="{{ $bis->nomor_bis }}" data-bis-id="{{ $bis->bis_id }}">
	...
	<div class="baris">
		...
		<div class="kursi @if(isset($kursi[$bis->nomor_bis]['7E'])) {{ $kursi[$bis->nomor_bis]['7E'] }} @endif" @if(isset($penumpang[$bis->nomor_bis]['7E'])) data-toggle="tooltip" title="<div class='box-tooltip'><p>{{ $penumpang[$bis->nomor_bis]['7E'].' ('.$telephone[$bis->nomor_bis]['7E'].')' }}</p><p>Asal: {{ $asal[$bis->nomor_bis]['7E'] }}</p><p>Tujuan: {{ $tujuan[$bis->nomor_bis]['7E'] }}</p></div>" @endif>
			<input type="checkbox" value="7E" name="kursi[]" class="cek-kursi">7E
		</div>
	</div>
</div>